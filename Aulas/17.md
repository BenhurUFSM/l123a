...
```c
  ...
  char c[5] = "12345"; // nao ok, o vetor de 5 posições será inicializado com os caracteres, mas não será uma string porque nao terá '\0' final
...
No caso do scanf, ele pula caracteres espaço (e fim de linha, tabulação), e coloca no vetor os caracteres da entrada que não são espaço, até encontrar um caractere espaço, que não é colocado no vetor. A string é então terminada pelo '\0'. O scanf não tem como fazer verificação do tamanho do vetor, então pode escrever fora do vetor caso tenha na entrada mais caracteres que a capacidade do vetor. Por causa desse problema, não é recomendado usar scanf para ler strings.
Pode-se limitar o número de caracteres a serem lidos pelo scanf, com um formato como `%29s`; nesse caso, o scanf lerá no máximo 29 caracteres, e não causará estouro ao ler uma string para um vetor com tamanho de pelo menos 30 char.
...
```c
  ...
  printf("Seu nome é %s.\n", nome);
```
Outro cuidado a tomar no caso de usar scanf é que como se está passando um vetor como argumento, não se deve colocar o `&` antes do nome do vetor, diferente dos demais tipos de dados. O fgets no código acima poderia ser substituído por:
```c
  scanf("%19s", nome);
```

Incluindo `<string.h>` em um programa, se tem acesso a um conjunto de funções para o manuseio de strings.
Entre elas:
- `strlen`: recebe uma string (o vetor que contém a string) e retorna o tamanho dessa string (o número de caracteres que ela contém). Para isso, conta todos os caracteres que antecedem o '\0'.
...
- `strcat`: recebe dois vetores, contendo uma string cada, e concatena a segunda string à primeira (acrescenta os caracteres da segunda string ao final da primeira). É responsabilidade do programador garantir que o primeiro vetor tem espaço suficiente para a string que irá conter.
...
1. Implemente as funções acima. Faça um programa para testá-las, com as funções oficiais e com as tuas versões. Como contribuição, segue uma possível implementação para a primeira delas (com o nome alterado para não dar conflito com as oficiais):
    ...
    ```
   O bom observador pode ter percebido que `i` e `tam` na função acima são sempre iguais. Daria para economizar uma variável.
